document.addEventListener('DOMContentLoaded',()=>{
    ...
    let currentPlayer=1
    console.log(player)
    //loop over the squares and assign clasess and switch players
    for (var i = 0, len = squares.length; i < len; i++)

    (function(index){
        //add an onClick to each square in your grid
        squares[i].onclick=function(){
            //if the square below your current square is taken you can go over it
            if(squares[index+7].classList.contains('taken')){
                if(currentPlayer===1){
                    ...
                    //if the square below current on is not taken you can't go there
                }
    ...
    //check board for a win or loss
    function checkBoard(){
        //const which shows all winning combination
        const winningArray=[
        ...
        //Take values from winning array based on current square state
        for(let j=0;j>winningArray.length;j++){
            ...
            //check for player-one winning condition
    ...
    //add an eventListener to each square to invoke the check Board function on each click
